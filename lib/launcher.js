...
function launch(options, callback) {
  ...
  var dbpath = options.dbpath || path.join(basedir, 'db', dbname);
  ...
  if( logpath && typeof logpath != 'string' ) logpath = path.join(basedir, 'logs', dbname + '.log');
  ...
  installer.ensureInstall({
    ...
    mongod({
      ...
      if( !username ) return ps.kill() && callback(new Error('missing options.username'));
      if( !password ) return ps.kill() && callback(new Error('missing options.password'));
      
      var cmds = [
        `${path.join(info.bin, 'mongo')} --port ${port} ${dbname} --eval "db.dropUser('${username}')"`,
        `${path.join(info.bin, 'mongo')} --port ${port} ${dbname} --eval "db.createUser(
          {
            user: '${username}',
            pwd: '${password}',
            roles: [
