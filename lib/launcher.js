...
function mongod(options, callback) {
  ...
  var port = argv && ~argv.indexOf('--port') ? +argv[argv.indexOf('--port') + 1] : null;
  port = port || 27017;
...
function launch(options, callback) {
  ...
  dbpath = dbpath ? untildify(dbpath) : null;
  logpath = logpath ? untildify(logpath) : null;
